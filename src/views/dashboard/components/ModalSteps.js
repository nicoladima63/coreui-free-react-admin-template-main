...
import { StepsService, TasksService, TodoService, UsersService } from '../../../services/api';
...
const ModalSteps = ({ visible, onClose, task, focusedStepId }) => {
  ...
  // Handlers
  const handleToggleStepOld = useCallback(async (stepId, completed) => {
    try {
        ...
        if (nextStep) {
          ...
          try {
            if (nextStep.user.name === 'Tutti') {
              ...
              const allUsers = await UsersService.getUsers(); // Supponendo che esista questa API
